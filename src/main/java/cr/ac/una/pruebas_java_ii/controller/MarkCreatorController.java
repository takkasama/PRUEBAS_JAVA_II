...
public class MarkCreatorController extends Controller implements Initializable {
    ...
    @FXML
    private ToggleButton tglShiftDay;
    ...
    @FXML
    private ToggleButton tglShiftNight;
    ...
    @FXML
    private void onActionEntry(ActionEvent event) {
        if(employee == null || (employee.getId() < 1 || employee.getId() == null))
            new Mensaje().showModal(Alert.AlertType.ERROR, "Empleado no valido, favor confirmar id nuevamente"
                    , getStage(), "onActinoEntry");
        else if (loadEmployee())
            saveMark();
        else 
            new Mensaje().showModal(Alert.AlertType.ERROR, "Error al guardar la marca o ingresar el id empleado", getStage(), "");
    }
    ...
    @FXML
    private void onKeyPressedTxtIdEmployee(KeyEvent event) {
        loadEmployee();
        lblemployeeInfo.setText("Employee : " +  employee.getName() + " " + employee.getLastName());
    }
    ...
    private void saveMark(){
        try {
            ...
            if (!res.getEstado()) {
                new Mensaje().showModal(Alert.AlertType.ERROR, "Error to save the mark", getStage(), "saveMark");
                return;
            }
    ...
    private boolean loadEmployee(){
        try{
            ...
            Respuesta res = employeeService.getEmployee(Long.valueOf(txtEmployeeId.getText()));
          
            if(!res.getEstado())
                return false;
            
            employee = (EmployeeDTO) res.getResultado("Employee");
            return true;
        }catch (Exception e) {
            Logger.getLogger(MarkCreatorController.class.getName()).log(Level.SEVERE, "Error to load employee", e);
            new Mensaje().showModal(Alert.AlertType.ERROR, "Error to loadEmployee", getStage(), "loadEmployee");
            return false;
        }
    ...
    private class ButtonCell extends ListCell<MarkDTO> {
        ...
        public ButtonCell() {
            ...
            cellButton.setOnAction((t) -> {
                if (!new Mensaje().showConfirmation("RemoveMark", getStage(), "Are you Shure?")) {
                    return;
                }
